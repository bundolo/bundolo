...
public class GlobalStorage {
	...
	//TODO design a way to refresh labels and navigation when needed, possibly after they are updated
	...
	public synchronized Map<String, String> getLabelsForLocale(String locale) {
		...
		if (localeLabels == null) {
			//this is the first request so we have to initialize locales
			localeLabels = new HashMap<String, Map<String, String>>();
			//get all locales and set labels for them
			...
			if ((locales != null) && (locales.size() > 0)) {
				for(String tempLocale: locales) {
					...
					for(Content content: contents) {
						...
						logger.log(Constants.SERVER_DEBUG_LOG_LEVEL, "tempLocale: " + tempLocale + ", label: " + content.getText());
					}
	...
	private List<Object> getNavigationPageWithChildren(Page page) {
		logger.log(Constants.SERVER_DEBUG_LOG_LEVEL, "getNavigationPageWithChildren");
		...
		if (page != null) {
			...
			if (descriptionContent != null) {
				...
				logger.log(Constants.SERVER_DEBUG_LOG_LEVEL, "getNavigationPageWithChildren description found: " + pageDTO.getDescriptionContent().getName());				
			} else {
				logger.log(Constants.SERVER_WARN_LOG_LEVEL, "getNavigationPageWithChildren description not found");
			}
			result.add(pageDTO);
