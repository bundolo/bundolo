...
public class UserProfileServiceImpl implements UserProfileService {
	...
	public void saveUserProfile(UserProfileDTO userProfileDTO) throws Exception {
		...
		ContentDTO descriptionContent = new ContentDTO(null, userProfile.getUsername(), null, ContentKindType.user_description, null, "", SessionUtils.getUserLocale(),
...
//				userProfile.setPassword(hashResult.get(0));
		...
		if (nonce != null) {
			...
			String activationUrl = "http://127.0.0.1:8888/Bundolo.html?gwt.codesvr=127.0.0.1:9997&email="+userProfileDTO.getEmail()+"&nonce=" + nonce + "#activate";
			//TODO
			String emailBody = "Hi "+userProfile.getUsername()+"\n\n" +
					...
					nonce;
			//TODO send email
			logger.log(Constants.SERVER_DEBUG_LOG_LEVEL, "emailBody: " + emailBody);
		}
	}
	...
	public UserDTO updateUserProfile(UserProfileDTO userProfileDTO) throws Exception {
		...
		if (descriptionContent != null) {
			if (descriptionContent.getContentId() == null) {
				Long descriptionContentId = contentService.saveContent(userProfileDTO.getDescriptionContent()); //this is not going to be used at the moment, but it will when we add more locales, to add descriptions in different languages
		...
		if ((Utils.hasText(userProfileDTO.getPassword())) && (!userProfile.getPassword().equals(userProfileDTO.getPassword()))) {
		...
		if ((Utils.hasText(userProfileDTO.getNewEmail())) && (!userProfile.getEmail().equals(userProfileDTO.getNewEmail()))) {
		...
		if ((Utils.hasText(userProfileDTO.getNewEmail())) && (!userProfile.getEmail().equals(userProfileDTO.getNewEmail()))) {
			//TODO compose this url
			String activationUrl = "http://127.0.0.1:8888/Bundolo.html?gwt.codesvr=127.0.0.1:9997&email="+userProfile.getNewEmail()+"&nonce=" + userProfile.getNonce() + "#activate";
			//TODO
			String emailBody = "Hi "+userProfile.getUsername()+"\n\n" +
					"Someone, probably you, registered at Bundolo\n" +
					"Please use the following URL to complete the registration.\n\n" +
		...
		Content descriptionContentUpdated = contentDAO.findContentForLocale(userProfile.getDescriptionContentId(), ContentKindType.user_description, SessionUtils.getUserLocale());
		if (descriptionContentUpdated != null) {
			result.setDescriptionContent(DozerBeanMapperSingletonWrapper.getInstance().map(descriptionContentUpdated, ContentDTO.class));
		}
		return result;
	}
	
//	@Transactional(propagation=Propagation.REQUIRED, rollbackFor=Exception.class)
//	private UserDTO updateUserProfileByUsername(UserProfileDTO userProfileDTO) throws Exception {
//		UserDTO result = null;
//		UserProfile userProfile = userProfileDAO.findByField("username", userProfileDTO.getUsername());
//		if(userProfile == null) {
//			throw new Exception("no permission exception");
//		}
//		userProfileDAO.merge(userProfile);
...
//	}
	...
	public UserDTO login(String username, String password, Boolean rememberMe) throws Exception {
		...
		UserDTO result = null;
		//TODO implement this. use owasp code from https://www.owasp.org/index.php/Hashing_Java
		//some of the required classes might not be visible in gwt so read this http://code.google.com/p/google-web-toolkit-incubator/wiki/LoginSecurityFAQ
		...
		if ((userProfile != null) && (UserProfileStatusType.active.equals(userProfile.getUserProfileStatus()))) {
			//TODO once password hashing is implemented, we will check password differently
			if (password.equals(userProfile.getPassword())) {
				...
				UserProfileDTO userProfileDTO = DozerBeanMapperSingletonWrapper.getInstance().map(userProfile, UserProfileDTO.class);
				
				Content descriptionContent = contentDAO.findContentForLocale(userProfile.getDescriptionContentId(), ContentKindType.user_description, SessionUtils.getUserLocale());
				if (descriptionContent != null) {
					ContentDTO descriptionContentDTO = DozerBeanMapperSingletonWrapper.getInstance().map(descriptionContent, ContentDTO.class);
				...
				if (rememberMe) {
					...
					String rememberMeHashDecoded = SecurityUtils.getHashWithoutSalt(userProfileDTO.getUsername()+":"+calendar.getTimeInMillis()+":"+userProfile.getSalt());
					String rememberMeCompleteDecoded = userProfileDTO.getUsername()+":"+calendar.getTimeInMillis() + ":" + rememberMeHashDecoded;
					result.setRememberMeCookie(Base64.encodeBase64URLSafeString(rememberMeCompleteDecoded.getBytes("UTF-8")));
				}
			...
			if(userProfile != null) {
				...
				if (Utils.hasText(userProfile.getNewEmail())) {
					if(userProfile.getNewEmail().equals(email)) {
						serverNonce = SecurityUtils.getHashWithoutSalt(userProfile.getNewEmail()+":"+userProfile.getSalt());
					...
					if(userProfile.getEmail().equals(email)) {
						serverNonce = SecurityUtils.getHashWithoutSalt(userProfile.getEmail()+":"+userProfile.getSalt());
						if (serverNonce.equals(nonce)) {
	...
	public UserDTO validateSession() throws Exception {
		//attempt to login user from sessionId cookie. if not, look for remember me cookie.
		
		//TODO according to http://code.google.com/p/google-web-toolkit-incubator/wiki/LoginSecurityFAQ this might not be completely secure
		...
		if (result == null) {
			...
			if (Utils.hasText(rememberMe)) {
				...
				if (components != null && components.length == 3) {
					...
					if (userProfile != null) {
						String rememberMeHashDecoded = SecurityUtils.getHashWithoutSalt(components[0]+":"+components[1]+":"+userProfile.getSalt());
						...
						if (Utils.hasText(rememberMeHashDecoded) && rememberMeHashDecoded.equals(components[2])) {
							logger.log(Constants.SERVER_DEBUG_LOG_LEVEL, "validateSession, user verified from rememberMe cookie.");
		...
		//TODO probably, in case of result != null do similar thing like in login, update last ip and login date, save session cookie and session attribute
		return result;
	}
	...
	public UserDTO findUserByUsername(String username) {
		...
		if(userProfile != null) {
			...
			Content descriptionContent = contentDAO.findContentForLocale(userProfile.getDescriptionContentId(), ContentKindType.user_description, SessionUtils.getUserLocale());
			if (descriptionContent != null) {
				userDTO.setDescriptionContent(DozerBeanMapperSingletonWrapper.getInstance().map(descriptionContent, ContentDTO.class));
			}
			result =  userDTO;
		}
		return result;
