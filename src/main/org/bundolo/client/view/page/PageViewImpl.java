...
public class PageViewImpl extends Composite implements PageView {
    ...
    @UiField
    ConditionalPanel pageControlsPanel;
	...
	// addContentButton = new PushButton(new
	// Image(LocalStorage.getInstance().getIconResource().commentPlus()));
	// addContentButton.setTitle(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.add_comment));
	controlButton = new ToggleButton(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.detach),
		LocalStorage.getInstance().getMessageResource().getLabel(LabelType.attach));
	...
	// pageData.add(new
	// Label(presenter.getCurrentPage().getDescriptionContent().getName()));
	// pageContent.add(new
	// HTML(presenter.getCurrentPage().getDescriptionContent().getText()));
	ContentPresenter contentPresenter = (ContentPresenter) LocalStorage.getInstance().getPresenter(
		PresenterName.content.name());
	...
	protected void onUpdate(double progress) {
	    ...
	    // sidebar.setWidth((int) (progress * sidebarWidth + hoverBarWidth)
	    // + "px");
	...
	pageSpecific.clear();
	org.bundolo.client.presenter.Presenter pageSpecificPresenter = presenter
		.getPageSpecificPresenter(navigationPages);
