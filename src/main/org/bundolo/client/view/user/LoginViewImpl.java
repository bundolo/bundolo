...
public class LoginViewImpl extends Composite implements LoginView {
	...
	addLabelHandlers();
	usernameLabel.setText(LocalStorage.getInstance().getMessageResource()
		.getLabel(LabelType.login_username_or_email));
	...
	presenter.getValidator().addValidators(
		LabelType.login_username_or_email.name(),
		new NotEmptyValidatorExtended(username).addActionForFailure(new StyleAction("validationFailedBorder"))
			.addActionForFailure(
				new LabelTextActionExtended(usernameLabel, LabelType.login_username_or_email)),
		new RegularExpressionValidator(username, "^(?!"
			+ LocalStorage.getInstance().getMessageResource().getLabel(LabelType.anonymous_user) + "$).*",
			LocalStorage.getInstance().getMessageResource()
				.getLabel(LabelType.validator_username_forbidden)).addActionForFailure(
			new StyleAction("validationFailedBorder")).addActionForFailure(
			new LabelTextActionExtended(usernameLabel, LabelType.login_username_or_email)));
	presenter.getValidator().addValidators(
		LabelType.login_password.name(),
		new NotEmptyValidatorExtended(password).addActionForFailure(new StyleAction("validationFailedBorder"))
			.addActionForFailure(new LabelTextActionExtended(passwordLabel, LabelType.login_password)));
	presenter.getValidator().addValidators(
		LabelType.login_email.name(),
		new NotEmptyValidatorExtended(email).addActionForFailure(new StyleAction("validationFailedBorder"))
			.addActionForFailure(new LabelTextActionExtended(emailLabel, LabelType.login_email)),
		new EmailValidator(email).addActionForFailure(new StyleAction("validationFailedBorder"))
			.addActionForFailure(new LabelTextActionExtended(emailLabel, LabelType.login_email)));
    }
	...
	currentUserMessage.setText(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.logged_in) + " "
		+ username);
