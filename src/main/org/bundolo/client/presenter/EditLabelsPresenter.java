...
public class EditLabelsPresenter implements Presenter, EditLabelsView.Presenter {
	...
	public void go(final HasWidgets container) {
		...
		//TODO this check is performed before user is logged in, if he just came and has session still open
	...
	private void doSave() {
		...
		contentService.saveLabels(view.getLabels(), new AsyncCallback<Void>() {
			public void onFailure(Throwable caught) {
            	...
            	eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.labels_saving_failed)));
			}
			@Override
			public void onSuccess(Void result) {
				...
				eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.labels_saved)));
			}
		});
	}
