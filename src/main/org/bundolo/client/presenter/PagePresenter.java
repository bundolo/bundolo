...
public class PagePresenter implements Presenter, PageView.Presenter {  
  ...
  private ContentServiceAsync contentService;
  private final HandlerManager eventBus;
  private final PageView view;
  Presenter pageSpecificPresenter;
  //private List<Object> navigationPages;
  
  public PagePresenter(PageServiceAsync pageService, ContentServiceAsync contentService, HandlerManager eventBus, PageView view) {
  ...
  public void onEditPageButtonClicked() {
	  EditPagePresenter editPagePresenter = (EditPagePresenter)LocalStorage.getInstance().getPresenter(PresenterName.editPage.name());
  ...
  public void go(final HasWidgets container) {
    ...
    displayMainPanel();
    //TODO navigation should be refreshed sometimes. to be defined. probably some notification driven refresh when pages are modified    
    displayPageSpecific();
  }
  
  //TODO instead of manually finding current page, we might use MenuBar.getSelectedItem() probably
  ...
  private void deleteCurrentPage() {
	  ...
	  pageService.deletePage(currentPage.getPageId(), new AsyncCallback<Void>() {
		  			...
		  			public void onFailure(Throwable caught) {
		  				...
		  				eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.page_deleting_failed)));
		  			}
		  			...
		  			public void onSuccess(Void result) {
		  				...
		  				eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.page_deleted)));
		  			}}
		      	);
  }
...
private void doPageUpdated() {
	//TODO decide where to go: page just saved might be unreachable normally. in case of adding it makes sense to go to previous page. in case of update it's unclear
	Utils.setHistoryTokenIfNeeded(PresenterName.home);
	LocalStorage.getInstance().refreshNavigationPages();
}
...
//    		logger.severe("looping: " + tempPage.getName().toLowerCase().replace(" ", "-"));
//    		if (tempPage.getName().toLowerCase().replace(" ", "-").equals(History.getToken())) {
...
public void displayStatusMessage(String statusMessage) {
	StatusPresenter statusPresenter = (StatusPresenter)LocalStorage.getInstance().getPresenter(PresenterName.status.name());
...
private PageDTO findPageByDisplayName(List<Object> navigationData, String displayName) {
	...
	private void displayPageSpecific() {
	    ...
	    if ((currentPage == null) || (!currentPage.getDescriptionContent().getKind().name().equalsIgnoreCase(History.getToken()))) {
