...
public class LoginPresenter implements Presenter, LoginView.Presenter {
	...
	public void onUserProfileButtonClicked() {
		UserProfilePresenter userPresenter = (UserProfilePresenter)LocalStorage.getInstance().getPresenter(PresenterName.user.name());
	...
	private void doLogin() {
		...
		userProfileService.login(view.getUsername(), view.getPassword(), view.getRememberMe(), new AsyncCallback<UserDTO>() {
			@Override
			public void onFailure(Throwable caught) {
				logger.log(Level.SEVERE, "Error authenticating user: ", caught);
				eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.login_failed)));
			}
			@Override
			public void onSuccess(UserDTO result) {
				...
				if (result != null) {
					...
					if (view.getRememberMe()) {
						Cookies.setCookie(Constants.REMEMBER_ME_COOKIE_NAME, result.getRememberMeCookie(), Utils.addDays(new Date(), 14));
					}
					userDTO = result;
				...
				//this is plain cookie: String msg = Cookies.getCookie("MyCookie"); Cookies.setCookie("MyCookie",msg);
			}
		});
	}
	...
	private void doLogout() {
		userProfileService.logout(new AsyncCallback<Void>() {
			...
			public void onFailure(Throwable caught) {
				...
				eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.logout_failed)));
			}
			@Override
	...
	public void onSendPasswordButtonClicked() {
		//TODO
		Window.alert("onSendPasswordButtonClicked");
	}
