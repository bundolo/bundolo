...
public class EditMessagePresenter implements Presenter, EditMessageView.Presenter {
	...
	public EditMessagePresenter(UserProfileServiceAsync userProfileService, HandlerManager eventBus, EditMessageView view) {
	...
	private void doSend() {
		...
		//TODO validation		
		userProfileService.sendMessage(view.getTitle(), view.getTitle(), recipientUserDTO.getUsername(), new AsyncCallback<Boolean>() {
				public void onFailure(Throwable caught) {
					if(caught instanceof ValidationException) {
		            	...
		            	eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.message_sending_failed)));
		            }
				}
				@Override
				public void onSuccess(Boolean result) {
					...
					eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance().getMessageResource().getLabel(LabelType.message_sent)));
				}
			});
	}
