...
public class LocalStorage {
	...
	page, addPage, editPage, content, addComment, editComment, user, addUser, editUser, activate, home, login, status, lists, addList, editList, forum, addForumPost, editForumPost, serials, addSerial, editSerial, addEpisode, editEpisode, authors, texts, addText, editText, news, addNews, editNews, contests, addContest, editContest, connections, addConnection, editConnection, addMessage, sidebar, editLabels
    };
	...
	// TODO there is a chance that first time LocalStorage is instantiated
	// call to retrieve navigationPages has not finished yet. think of a way
	// to avoid this.
	...
	// make first letter lower case. this is a bit of a hack because we need
	// to keep page kind type names lower case for consistency with other
	// enums,
	...
	try {
		...
		case lists:
		    ...
		    result = new ItemListPresenter(itemListService, contentService, userProfileService, contestService,
			    connectionService, eventBus, itemListViewImpl);
		    break;
		case addList:
		case editList:
    ...
    public Navigation getNavigation(int width, int height) {
	if (navigation == null) {
	...
	if (result == null) {
	    ...
	    itemListService.findItemListByName(itemListName.getItemListName(), null, null,
			...
			public void onFailure(Throwable caught) {
			    ...
			    eventBus.fireEvent(new DisplayNotificationEvent(LocalStorage.getInstance()
				    .getMessageResource().getLabel(LabelType.database_error)));
			}
	    ...
	    // this is a bit hacky, we want lists in a map to be generic, but
	    // the method should return query with params inserted
