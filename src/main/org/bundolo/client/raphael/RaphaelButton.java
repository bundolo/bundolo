...
public class RaphaelButton {
	...
	private Logger logger = Logger.getLogger(RaphaelButton.class.getName());
	...
	public RaphaelButton(String iconPathString, final int positionX, int positionY, final PageDTO pageDTO) {
		this.iconPathString = iconPathString;
		this.positionX = positionX;
		this.positionY = positionY;
		circle = LocalStorage.getInstance().getNavigation().new Circle(centerOffset, centerOffset, INITIAL_RADIUS);
		PathBuilder pathBuilder = new PathBuilder();
		pathBuilder.append(iconPathString);
		iconPath = LocalStorage.getInstance().getNavigation().new Path(pathBuilder);
		...
		iconPath.addDomHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				((PageView.Presenter) LocalStorage.getInstance().getPresenter(PresenterName.page.name())).onNavigatePageButtonClicked(pageDTO);
			}
		}, ClickEvent.getType());
...
//				this.y = y;
			...
			public void onEnd() {
				...
				int canvasWidth = LocalStorage.getInstance().getNavigation().getOffsetWidth();
				int canvasHeight = LocalStorage.getInstance().getNavigation().getOffsetHeight();
//				logger.log(Level.SEVERE, "canvasWidth: " + canvasWidth + ", canvasHeight: " + canvasHeight);
				if ((button.positionX + dx < 0) || (button.positionX + dx + 2*getZoomRadius() > canvasWidth) || (button.positionY + dy < 0) || (button.positionY + dy + 2*getZoomRadius() > canvasHeight)) {
					return;
				}
				//refuse drag if buttons would overlap
				if (((button.connection1 == null) || (!button.connection1.buttonsTooClose(dx, dy, 1))) && ((button.connection2 == null) || (!button.connection2.buttonsTooClose(dx, dy, -1)))) {			
					button.positionX += dx;
					button.positionY += dy;
					button.circle.translate(dx, dy);
					button.iconPath.translate(dx, dy);
					
//					logger.log(Level.SEVERE, "onEnd: " + button.getPositionXWithOffset() + ", " + button.getPositionYWithOffset());
