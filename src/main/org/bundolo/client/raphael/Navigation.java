...
public class Navigation extends Raphael {
		...
		for (Object navigationEntry : navigationData) {
			...
			if (navigationEntry instanceof List) {
				...
				if (firstListEntry instanceof PageDTO) {
					...
					currentButton = new RaphaelButton(IconPathType.getByName(
						((PageDTO) firstListEntry).getKind().name()).getIconPath(), positionX,
						positionY, (PageDTO) firstListEntry);
					if (previousButton != null) {
					    previousButton.setNeighbour1(currentButton);
					}
					previousButton = currentButton;
					// addIcon(getIconPathForPage((PageDTO)
					// firstListEntry), positionX,
					// positionY);
					positionX += 70;
					positionY = heightCenter + (Random.nextBoolean() ? 1 : -1)
						* Random.nextInt(heightCenter);
				    } else {
					currentButton = new RaphaelButton(IconPathType.getByName(
						((PageDTO) firstListEntry).getKind().name()).getIconPath(), positionX,
						positionY, (PageDTO) firstListEntry);
			    ...
			    currentButton = new RaphaelButton(IconPathType.getByName(pageDTO.getKind().name())
				    .getIconPath(), positionX, positionY, pageDTO);
			    if (previousButton != null) {
				previousButton.setNeighbour1(currentButton);
			    }
			    previousButton = currentButton;
			    // addIcon(getIconPathForPage(pageDTO), positionX,
			    // positionY);
			    positionX += 70;
    ...
    /*
     * private String getIconPathForPage(PageDTO pageDTO) { String result = "";
     * switch (pageDTO.getKind()) { case home: result = IconPaths.home; break;
     * case forum: result =
     ...
     * ; break; case contests: result = IconPaths.view; break; case events:
     * result =
     ...
     * ; break; case authors: result = IconPaths.users; break; case texts:
     * result =
     ...
     * ; break; case lists: result = IconPaths.folder; break; case connections:
     * result = IconPaths.link; break; case news: result = IconPaths.talkq;
     * break; case serials: result =
     ...
     * ; break; case about: result = IconPaths.i; break; case custom: result =
     * IconPaths.question; break; default: result = IconPaths.question; break; }
     * return result; }
     */
}
