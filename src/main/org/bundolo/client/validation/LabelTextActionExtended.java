...
/**
 * Action that sets the specified error message on the specified label.
 * 
...
public class LabelTextActionExtended extends ValidationAction<Object> {
	...
	this(errorLabel);
	defaultLabelText = LocalStorage.getInstance().getMessageResource()
		.getLabel(labelType);
    }
	...
	String prefix = "";
	if (withPropertyName && error.propertyName != null
		&& !error.propertyName.trim().equals("")) {
	    if (!localeInfo.isRTL())
