...
public class RaphaelButtonWidget extends Raphael implements HasClickHandlers {
    ...
    private final Logger logger = Logger.getLogger(RaphaelButtonWidget.class
	    .getName());
	...
	circlePathBuilder.append("M" + centerOffset + ","
		+ (centerOffset - INITIAL_RADIUS) + "A" + INITIAL_RADIUS + ","
		+ INITIAL_RADIUS + ",0,1,1," + (centerOffset - 0.1) + ","
		+ (centerOffset - INITIAL_RADIUS) + "z");
	...
	initialAttrs.put("stroke", new JSONString(INITIAL_STROKE));
	initialAttrs.put("scale", new JSONNumber(INITIAL_ICON_SCALE));
	// initialAttrs.put("transform", new JSONString("t" + centerOffset + ","
	// + centerOffset));
	// circle.attr(initialAttrs);
	...
	if (Utils.hasText(modifier)) {
	    PathBuilder modifierPathBuilder = new PathBuilder();
	    modifierPathBuilder.append(IconPathType.getByName(modifier)
		    .getIconPath());
	    modifierPath = new Path(modifierPathBuilder);
	    initialAttrs.put("scale", new JSONNumber(INITIAL_MODIFIER_SCALE));
	...
	MouseOverHandler mouseOverHandler = new MouseOverHandler() {
		...
		if (modifierPath != null) {
		    jSONObject.put("scale", new JSONNumber(
			    MODIFIER_ZOOM_IN_SCALE));
		    modifierPath.animate(jSONObject, 100);
		}
	    }
	};
	iconPath.addDomHandler(mouseOverHandler, MouseOverEvent.getType());
	circlePath.addDomHandler(mouseOverHandler, MouseOverEvent.getType());
	...
	MouseOutHandler mouseOutHandler = new MouseOutHandler() {
		...
		if (modifierPath != null) {
		    jSONObject.put("scale", new JSONNumber(
			    INITIAL_MODIFIER_SCALE));
		    modifierPath.animate(jSONObject, 100);
		}
	    }
	};
	iconPath.addDomHandler(mouseOutHandler, MouseOutEvent.getType());
	circlePath.addDomHandler(mouseOutHandler, MouseOutEvent.getType());

	setTitle(LocalStorage.getInstance().getMessageResource()
		.getLabel(LabelType.valueOf(label)));
    }
